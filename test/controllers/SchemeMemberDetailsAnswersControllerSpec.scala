...
class SchemeMemberDetailsAnswersControllerSpec extends ControllerBaseSpec {
  ...
  private def onPageLoad(checkOrChange: CheckOrChange) =
    controllers.routes.SchemeMemberDetailsAnswersController.onPageLoad(srn, refineMV(1), checkOrChange)
  private def onSubmit(checkOrChange: CheckOrChange) =
    controllers.routes.SchemeMemberDetailsAnswersController.onSubmit(srn, refineMV(1), checkOrChange)
