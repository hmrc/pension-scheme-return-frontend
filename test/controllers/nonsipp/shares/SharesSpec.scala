...
class SharesSpec extends ControllerBaseSpec with ControllerBehaviours {
  ...
  private val cyaUrl = s"/pension-scheme-return/${srn.value}/check-answers-shares"
  private val changeCyaUrl = s"/pension-scheme-return/${srn.value}/change-check-answers-shares"
  private val nonCyaUrl = s"/pension-scheme-return/${srn.value}/some-other-shares-page"
  ...
  "CallOps" - {
    ...
    "isCyaPage" - {
      ...
      "must return true for a standard shares CYA page URL" in {
        cyaPageCall.isCyaPage mustBe true
      }

      "must return true for a change shares CYA page URL" in {
    ...
    "when the next page is not a CYA page" - {
      "and alwaysCompleted is false (default)" - {
        "must set the progress to InProgress" in {
          ...
          val updatedUserAnswers = result.futureValue
          updatedUserAnswers.get(SharesProgress(srn, index)) mustBe Some(
