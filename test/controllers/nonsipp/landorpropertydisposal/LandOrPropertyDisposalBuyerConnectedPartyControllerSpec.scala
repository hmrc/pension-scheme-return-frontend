...
class LandOrPropertyDisposalBuyerConnectedPartyControllerSpec extends ControllerBaseSpec {
  ...
  "LandOrPropertyDisposalSellerConnectedPartyController" - {
    ...
    val individualUserAnswers = defaultUserAnswers
      ...
      .unsafeSet(LandOrPropertyIndividualBuyerNamePage(srn, index, disposalIndex), buyersName)
    ...
    val companyUserAnswers = defaultUserAnswers
      ...
      .unsafeSet(CompanyBuyerNamePage(srn, index, disposalIndex), buyersName)
    ...
    val partnershipUserAnswers = defaultUserAnswers
      ...
      .unsafeSet(PartnershipBuyerNamePage(srn, index, disposalIndex), buyersName)
      ...
      case (testScenario, userAnswers) =>
        act.like(renderView(onPageLoad, userAnswers) { implicit app => implicit request =>
          injected[YesNoPageView]
            .apply(form(injected[YesNoPageFormProvider]), viewModel(srn, buyersName, index, disposalIndex, NormalMode))
          ...
          renderPrePopView(
            ...
            injected[YesNoPageView]
              .apply(
                ...
                viewModel(srn, buyersName, index, disposalIndex, NormalMode)
              )
          }.updateName(_ + s" as $testScenario")
        )
