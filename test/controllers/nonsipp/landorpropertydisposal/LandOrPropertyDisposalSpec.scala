...
class LandOrPropertyDisposalSpec extends ControllerBaseSpec with ControllerBehaviours {
  ...
  private val cyaUrl = s"/pension-scheme-return/${srn.value}/check-answers-land-property-disposal/"
  private val changeCyaUrl = s"/pension-scheme-return/${srn.value}/change-check-answers-land-property-disposal/"
  private val nonCyaUrl = s"/pension-scheme-return/${srn.value}/some-other-disposal-page"
  private val urlWithIndices = s"/pension-scheme-return/${srn.value}/check-answers-land-property-disposal/1/1"
  private val urlMissingTrailingSlash = s"/pension-scheme-return/${srn.value}/check-answers-land-property-disposal"
  ...
  "CallOps" - {
    ...
    "isCyaPage" - {
      ...
      "must return true for a standard land or property disposal CYA page URL" in {
        cyaPageCall.isCyaPage mustBe true
      }

      "must return true for a change land or property disposal CYA page URL" in {
  ...
  "saveProgress" - {
    ...
    "when the next page is a CYA page" - {
      "must set the progress to Completed" in {
        ...
        updatedUserAnswers.get(LandOrPropertyDisposalProgress(srn, index, disposalIndex)) mustBe Some(
