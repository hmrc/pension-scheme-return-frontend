...
class LandOrPropertySpec extends ControllerBaseSpec with ControllerBehaviours {
  ...
  "CallOps" - {
    ...
    "isCyaPage" - {
      ...
      "must return true for a standard land or property CYA page URL" in {
        cyaPageCall.isCyaPage mustBe true
      }

      "must return true for a change land or property CYA page URL" in {
  ...
  "saveProgress" - {
    ...
    "when the next page is a CYA page" - {
      "must set the progress to Completed" in {
        ...
        updatedUserAnswers.get(LandOrPropertyProgress(srn, index)) mustBe Some(SectionJourneyStatus.Completed)
      }
    }
