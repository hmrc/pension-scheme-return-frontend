...
import pages.nonsipp.moneyborrowed.MoneyBorrowedProgress
import viewmodels.models.SectionJourneyStatus

class MoneyBorrowedSpec extends ControllerBaseSpec with ControllerBehaviours {
  ...
  private val cyaUrl = s"/pension-scheme-return/${srn.value}/check-answers-money-borrowed"
  private val changeCyaUrl = s"/pension-scheme-return/${srn.value}/change-check-answers-money-borrowed"
  private val nonCyaUrl = s"/pension-scheme-return/${srn.value}/some-other-money-borrowed-page"
  ...
  "CallOps" - {
    ...
    "isCyaPage" - {
      ...
      "must return true for a standard money borrowed CYA page URL" in {
        cyaPageCall.isCyaPage mustBe true
      }

      "must return true for a change money borrowed CYA page URL" in {
  ...
  "saveProgress" - {
    ...
    "when the next page is a CYA page" - {
      "must set the progress to Completed" in {
        ...
        updatedUserAnswers.get(MoneyBorrowedProgress(srn, index)) mustBe Some(SectionJourneyStatus.Completed)
      }
    }
    ...
    "when the next page is not a CYA page" - {
      "and alwaysCompleted is false (default)" - {
        "must set the progress to InProgress" in {
          ...
          val updatedUserAnswers = result.futureValue
          updatedUserAnswers.get(MoneyBorrowedProgress(srn, index)) mustBe Some(
