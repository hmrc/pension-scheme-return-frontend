...
class BondsSpec extends ControllerBaseSpec with ControllerBehaviours {
  ...
  private val cyaUrl = s"/pension-scheme-return/${srn.value}/check-answers-bonds"
  private val changeCyaUrl = s"/pension-scheme-return/${srn.value}/change-check-answers-bonds"
  private val nonCyaUrl = s"/pension-scheme-return/${srn.value}/some-other-page"
  private val excludedCyaUrl = s"/pension-scheme-return/${srn.value}/check-answers-bonds/1/1"
  ...
  "CallOps" - {
    ...
    "isCyaPage" - {
      ...
      "must return true for a standard bonds CYA page URL" in {
        cyaPageCall.isCyaPage mustBe true
      }

      "must return true for a change bonds CYA page URL" in {
      ...
      "must return false for a CYA page URL with trailing indices that should be excluded" in {
        excludedCyaPageCall.isCyaPage mustBe false
      }
  ...
  "saveProgress" - {
    ...
    "when the next page is a CYA page" - {
      "must set the progress to Completed" in {
        ...
        updatedUserAnswers.get(BondsProgress(srn, index)) mustBe Some(SectionJourneyStatus.Completed)
      }
    }
    ...
    "when the next page is not a CYA page" - {
      "and alwaysCompleted is false (default)" - {
        "must set the progress to InProgress" in {
          val result = saveProgress(srn, index, userAnswers, nonCyaPageCall) // alwaysCompleted defaults to false
          val updatedUserAnswers = result.futureValue
          updatedUserAnswers.get(BondsProgress(srn, index)) mustBe Some(SectionJourneyStatus.InProgress(nonCyaPageCall))
        }
      }
