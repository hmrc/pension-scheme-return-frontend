...
class OtherAssetsHeldSpec extends ControllerBaseSpec with ControllerBehaviours {
  ...
  private val cyaUrl = s"/pension-scheme-return/${srn.value}/check-answers-other-assets"
  private val changeUrl = s"/pension-scheme-return/${srn.value}/change-asset"
  private val nonCyaUrl = s"/pension-scheme-return/${srn.value}/some-other-asset-page"
  ...
  "CallOps" - {
    ...
    "isCyaPage" - {
      ...
      "must return true for a standard other assets CYA page URL" in {
        cyaPageCall.isCyaPage mustBe true
      }

      "must return true for a change other assets page URL" in {
