...
class EmployerContributionsSpec extends ControllerBaseSpec with ControllerBehaviours {
  ...
  private val cyaUrl = s"/pension-scheme-return/${srn.value}/check-answers-employer-contributions/"
  private val changeUrl = s"/pension-scheme-return/${srn.value}/change-employer-contributions/"
  private val nonCyaUrl = s"/pension-scheme-return/${srn.value}/some-other-employer-contributions-page"
  private val urlWithIndices = s"/pension-scheme-return/${srn.value}/check-answers-employer-contributions/1/1"
  private val urlMissingTrailingSlash = s"/pension-scheme-return/${srn.value}/check-answers-employer-contributions"
  ...
  "CallOps" - {
    ...
    "isCyaPage" - {
      ...
      "must return true for a standard employer contributions CYA page URL" in {
        cyaPageCall.isCyaPage mustBe true
      }

      "must return true for a change employer contributions page URL" in {
  ...
  "saveProgress" - {
    ...
    "when the next page is a CYA page" - {
      "must set the progress to Completed" in {
        ...
        updatedUserAnswers.get(EmployerContributionsProgress(srn, index, secondaryIndex)) mustBe Some(
