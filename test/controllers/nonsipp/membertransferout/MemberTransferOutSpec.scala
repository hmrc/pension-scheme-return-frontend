...
import pages.nonsipp.membertransferout.MemberTransferOutProgress
import viewmodels.models.SectionJourneyStatus

class MemberTransferOutSpec extends ControllerBaseSpec with ControllerBehaviours {
  ...
  private val cyaUrl = s"/pension-scheme-return/${srn.value}/check-answers-transfers-out/"
  private val changeCyaUrl = s"/pension-scheme-return/${srn.value}/change-check-answers-transfers-out/"
  private val nonCyaUrl = s"/pension-scheme-return/${srn.value}/some-other-transfer-out-page"
  private val urlWithIndices = s"/pension-scheme-return/${srn.value}/check-answers-transfers-out/1/1"
  private val urlMissingTrailingSlash = s"/pension-scheme-return/${srn.value}/check-answers-transfers-out"
  ...
  "CallOps" - {
    ...
    "isCyaPage" - {
      ...
      "must return true for a standard transfer out CYA page URL" in {
        cyaPageCall.isCyaPage mustBe true
      }

      "must return true for a change transfer out CYA page URL" in {
  ...
  "saveProgress" - {
    ...
    "when the next page is a CYA page" - {
      "must set the progress to Completed" in {
        ...
        updatedUserAnswers.get(MemberTransferOutProgress(srn, index, secondaryIndex)) mustBe Some(
