...
class OtherAssetsDisposalSpec extends ControllerBaseSpec with ControllerBehaviours {
  ...
  private val cyaUrl = s"/pension-scheme-return/${srn.value}/check-answers-asset-disposal/"
  private val changeCyaUrl = s"/pension-scheme-return/${srn.value}/change-check-answers-asset-disposal/"
  private val nonCyaUrl = s"/pension-scheme-return/${srn.value}/some-other-asset-disposal-page"
  private val urlWithIndices = s"/pension-scheme-return/${srn.value}/check-answers-asset-disposal/1/1"
  private val urlMissingTrailingSlash = s"/pension-scheme-return/${srn.value}/check-answers-asset-disposal"
  ...
  "CallOps" - {
    ...
    "isCyaPage" - {
      ...
      "must return true for a standard asset disposal CYA page URL" in {
        cyaPageCall.isCyaPage mustBe true
      }

      "must return true for a change asset disposal CYA page URL" in {
  ...
  "saveProgress" - {
    ...
    "when the next page is a CYA page" - {
      "must set the progress to Completed" in {
        ...
        updatedUserAnswers.get(OtherAssetsDisposalProgress(srn, index, secondaryIndex)) mustBe Some(
