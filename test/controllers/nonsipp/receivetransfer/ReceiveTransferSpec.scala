...
class ReceiveTransferSpec extends ControllerBaseSpec with ControllerBehaviours {
  ...
  private val cyaUrl = s"/pension-scheme-return/${srn.value}/check-answers-transfers-in/"
  private val changeCyaUrl = s"/pension-scheme-return/${srn.value}/change-check-answers-transfers-in/"
  private val nonCyaUrl = s"/pension-scheme-return/${srn.value}/some-other-transfer-in-page"
  private val urlWithIndices = s"/pension-scheme-return/${srn.value}/check-answers-transfers-in/1/1"
  private val urlMissingTrailingSlash = s"/pension-scheme-return/${srn.value}/check-answers-transfers-in"
  ...
  "CallOps" - {
    ...
    "isCyaPage" - {
      ...
      "must return true for a standard transfer in CYA page URL" in {
        cyaPageCall.isCyaPage mustBe true
      }

      "must return true for a change transfer in CYA page URL" in {
  ...
  "saveProgress" - {
    ...
    "when the next page is a CYA page" - {
      "must set the progress to Completed" in {
        ...
        updatedUserAnswers.get(ReceiveTransferProgress(srn, index, secondaryIndex)) mustBe Some(
