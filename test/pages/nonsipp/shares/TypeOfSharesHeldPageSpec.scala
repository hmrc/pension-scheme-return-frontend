...
class TypeOfSharesHeldPageSpec extends PageBehaviours {
  ...
  "DidSchemeHoldAnySharesPage" - {
    ...
    "cleanup" - {
      ...
      "remove check return dates value when TypeOfSharesHeldPage changes" - {
        val updatedAnswers =
          userAnswers.set(TypeOfSharesHeldPage(srn, index), TypeOfShares.ConnectedParty).toOption.value
        updatedAnswers.get(WhyDoesSchemeHoldSharesPage(srn, index)) mustBe None
        updatedAnswers.get(WhenDidSchemeAcquireSharesPage(srn, index)) mustBe None
      }

      "do not remove check return dates value when TypeOfSharesHeldPage remains the same" - {
        val updatedAnswers =
        ...
        updatedAnswers.get(WhenDidSchemeAcquireSharesPage(srn, index)) mustBe Some(SchemeHoldShare.Acquisition)
      }
    }
  }
}
