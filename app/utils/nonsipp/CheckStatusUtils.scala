  ...
  def checkLoansRecord(
    ...
    lazy val allOtherAnswersPresent: Boolean = (
      ...
      case (Some(identityType), Some(_), Some(amountOfTheLoan), Some(_), Some(interestOnLoan), Some(_)) =>
        identitySubjectAnswersPresent(userAnswers, srn, recordIndex, LoanRecipient, identityType)
      case (_, _, _, _, _, _) =>
        false
    }
    ...
    identitySubject match {
      case LoanRecipient =>
        identityType match {
          ...
          case Other =>
            (
              userAnswers.get(OtherRecipientDetailsPage(srn, recordIndex, identitySubject)),
              userAnswers.get(RecipientSponsoringEmployerConnectedPartyPage(srn, recordIndex))
            ) match {
