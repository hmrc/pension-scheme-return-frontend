...
//
//      borrowedAmount = indexes.map(index =>
//        BorrowedAmountAndRatePage(srn, index) -> (Money(12.00, "12.00"), Percentage(2, "2"))
//      )
//
//      whenBorrowed = indexes.map(index => WhenBorrowedPage(srn, index) -> LocalDate.of(2000, 12, 12))
//
//      totalSchemeValue = indexes.map(index =>
//        ValueOfSchemeAssetsWhenMoneyBorrowedPage(srn, index) -> Money(12.00, "12.00")
//      )
//
//      whyBorrowed = indexes.map(index => WhySchemeBorrowedMoneyPage(srn, index) -> "Business")
//
...
//      ua3 <- isLenderConnectedParty.foldLeft(Try(ua2)) { case (ua, (page, value)) =>
