...
//        IdentityTypePage(srn, index, IdentitySubject.LoanRecipient) -> IdentityType.Individual
//      )
...
//      nino = indexes.map(index =>
//        IndividualRecipientNinoPage(srn, index) -> ConditionalYesNo.yes[String, Nino](Nino("SX123456A"))
//      )
...
//        DatePeriodLoanPage(srn, index) -> (LocalDate.of(1990, 12, 12), Money(121.00, "121.00"), 12)
//      )
//      loanAmount = indexes.map(index =>
...
//      loanInterest = indexes.map(index =>
...
//        )
//      )
//      securityGiven = indexes.map(index =>
//        SecurityGivenForLoanPage(srn, index) -> ConditionalYesNo.yes[Unit, Security](Security("securityGivenForLoan"))
//      )
//      arrearsPrevYears = indexes.map(index => ArrearsPrevYears(srn, index) -> true)
//      outstandingArrearsOnLoan = indexes.map(index =>
...
//      ua5 <- individualConnectedPartyStatus.foldLeft(Try(ua4)) { case (ua, (page, value)) =>
...
//      ua12 <- outstandingArrearsOnLoan.foldLeft(Try(ua11)) { case (ua, (page, value)) =>
//        ua.flatMap(_.set(page, value))
//      }
