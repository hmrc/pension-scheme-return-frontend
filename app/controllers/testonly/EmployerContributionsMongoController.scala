...
//      employerBusinessType = indexes.map(secondaryIndex =>
//        EmployerTypeOfBusinessPage(srn, index, secondaryIndex) -> IdentityType.UKCompany
//      )
//      employerContribution = indexes.map(secondaryIndex =>
//        TotalEmployerContributionPage(srn, index, secondaryIndex) -> Money(10.50)
//      )
//      employerCrn = indexes.map(secondaryIndex =>
//        EmployerCompanyCrnPage(srn, index, secondaryIndex) -> ConditionalYesNo.yes[String, Crn](Crn("1234567890"))
//      )
//      ua1 <- employerName.foldLeft(Try(userAnswers)) { case (ua, (page, value)) =>
