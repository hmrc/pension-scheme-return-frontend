...
class IndividualNameOfOtherAssetSellerController @Inject()(
  ...
  def onPageLoad(srn: Srn, index: Int, mode: Mode): Action[AnyContent] =
    identifyAndRequireData(srn) { implicit request =>
      val preparedForm = request.userAnswers.fillForm(IndividualNameOfOtherAssetSellerPage(srn, index.refined), form)
          ...
          value =>
            for {
              updatedAnswers <- Future
                .fromTry(request.userAnswers.set(IndividualNameOfOtherAssetSellerPage(srn, index.refined), value))
              nextPage = navigator
                .nextPage(IndividualNameOfOtherAssetSellerPage(srn, index.refined), mode, updatedAnswers)
