  ...
  def onPageLoad(srn: Srn, index: Int, mode: Mode): Action[AnyContent] = identifyAndRequireData(srn) {
    implicit request =>
      request.userAnswers.get(LandOrPropertyChosenAddressPage(srn, index.refined)).getOrRecoverJourney { address =>
        val preparedForm = request.userAnswers.fillForm(IsLandOrPropertyResidentialPage(srn, index.refined), form)
          ...
          value =>
            for {
              updatedAnswers <- Future
                .fromTry(request.userAnswers.set(IsLandOrPropertyResidentialPage(srn, index.refined), value))
              nextPage = navigator.nextPage(IsLandOrPropertyResidentialPage(srn, index.refined), mode, updatedAnswers)
