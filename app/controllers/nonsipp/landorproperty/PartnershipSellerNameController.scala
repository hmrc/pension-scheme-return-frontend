...
class PartnershipSellerNameController @Inject()(
  ...
  def onPageLoad(srn: Srn, index: Int, mode: Mode): Action[AnyContent] = identifyAndRequireData(srn) {
    implicit request =>
      val preparedForm = request.userAnswers.fillForm(PartnershipSellerNamePage(srn, index.refined), form)
          ...
          value =>
            for {
              updatedAnswers <- Future
                .fromTry(request.userAnswers.set(PartnershipSellerNamePage(srn, index.refined), value))
              nextPage = navigator.nextPage(PartnershipSellerNamePage(srn, index.refined), mode, updatedAnswers)
...
object PartnershipSellerNameController {
  ...
  def viewModel(srn: Srn, index: Max5000, mode: Mode): FormPageViewModel[TextInputViewModel] = FormPageViewModel(
    "partnershipSellerName.title",
    "partnershipSellerName.heading",
    TextInputViewModel(isFixedLength = true),
    routes.PartnershipSellerNameController.onSubmit(srn, index, mode)
  )
}
