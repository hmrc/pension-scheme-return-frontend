...
import pages.nonsipp.landorproperty.{LandOrPropertyChosenAddressPage, LandOrPropertyTotalCostPage, WhyDoesSchemeHoldLandPropertyPage}
...
class SharesCheckAndUpdateController @Inject()(
  ...
  def onSubmit(srn: Srn, index: Max5000): Action[AnyContent] = identifyAndRequireData(srn) { _ =>
    Redirect(routes.SharesCheckAndUpdateController.onPageLoad(srn, index))
  }
}
