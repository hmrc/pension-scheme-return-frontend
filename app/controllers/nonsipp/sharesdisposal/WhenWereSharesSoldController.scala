        ...
        request.userAnswers.get(CompanyNameRelatedSharesPage(srn, shareIndex.refined)).getOrRecoverJourney {
          companyName =>
            val preparedForm = request.userAnswers
              .get(WhenWereSharesSoldPage(srn, shareIndex.refined, disposalIndex.refined))
                ...
                value =>
                  for {
                    updatedAnswers <- Future
                      .fromTry(
                        request.userAnswers
                          .set(WhenWereSharesSoldPage(srn, shareIndex.refined, disposalIndex.refined), value)
                      )
                    nextPage = navigator.nextPage(
                      WhenWereSharesSoldPage(srn, shareIndex.refined, disposalIndex.refined),
                      mode,
                      updatedAnswers
                    )
