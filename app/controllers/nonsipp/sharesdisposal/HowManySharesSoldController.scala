    ...
    identifyAndRequireData(srn) { implicit request =>
      request.userAnswers.get(CompanyNameRelatedSharesPage(srn, shareIndex.refined)).getOrRecoverJourney {
        companyName =>
          Ok(
            view(
              form.fromUserAnswers(HowManySharesSoldPage(srn, shareIndex.refined, disposalIndex.refined)),
              viewModel(
                srn,
                shareIndex.refined,
              ...
              answer =>
                for {
                  updatedAnswers <- Future
                    .fromTry(
                      request.userAnswers
                        .set(HowManySharesSoldPage(srn, shareIndex.refined, disposalIndex.refined), answer)
                    )
                  nextPage = navigator.nextPage(
                    HowManySharesSoldPage(srn, shareIndex.refined, disposalIndex.refined),
                    mode,
                    updatedAnswers
                  )
