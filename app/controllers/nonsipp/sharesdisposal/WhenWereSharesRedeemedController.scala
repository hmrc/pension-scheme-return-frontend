        ...
        request.userAnswers.get(CompanyNameRelatedSharesPage(srn, shareIndex.refined)).getOrRecoverJourney {
          companyName =>
            val preparedForm = request.userAnswers
              .get(WhenWereSharesRedeemedPage(srn, shareIndex.refined, disposalIndex.refined))
              .fold(form(date))(form(date).fill)
            Ok(
              view(
                preparedForm,
                viewModel(srn, shareIndex.refined, disposalIndex.refined, companyName, mode)
              )
            )
        }
      }
                ...
                value =>
                  for {
                    updatedAnswers <- Future
                      .fromTry(
                        request.userAnswers
                          .set(WhenWereSharesRedeemedPage(srn, shareIndex.refined, disposalIndex.refined), value)
                      )
                    nextPage = navigator
                      .nextPage(
                        WhenWereSharesRedeemedPage(srn, shareIndex.refined, disposalIndex.refined),
                        mode,
                        updatedAnswers
                      )
