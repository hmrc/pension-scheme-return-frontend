          ...
          value =>
            for {
              updatedAnswers <- Future
                .fromTry(request.userAnswers.set(CompanyBuyerNamePage(srn, index, secondaryIndex), value))
              _ <- saveService.save(updatedAnswers)
            } yield Redirect(
              navigator.nextPage(CompanyBuyerNamePage(srn, index, secondaryIndex), mode, updatedAnswers)
            )
        )
    }
}
...
object CompanyNameOfSharesBuyerController {
  def form(formProvider: TextFormProvider): Form[String] = formProvider.text(
    ...
    "companyNameOfSharesBuyer.error.invalid"
  )
