        ...
        .fold(
          ...
          answer => {
            for {
              ...
              nextPage = navigator
                .nextPage(PartnershipBuyerNamePage(srn, index, disposalIndex), mode, updatedAnswers)
...
object PartnershipBuyerNameController {
  def form(formProvider: TextFormProvider): Form[String] = formProvider.name(
    "otherAssetsDisposal.partnershipBuyerName.error.required",
    "otherAssetsDisposal.partnershipBuyerName.error.length",
    "error.textarea.invalid"
  )

  def viewModel(srn: Srn, index: Max5000, disposalIndex: Max50, mode: Mode): FormPageViewModel[TextInputViewModel] =
    FormPageViewModel(
      Message("otherAssetsDisposal.partnershipBuyerName.title"),
      Message("otherAssetsDisposal.partnershipBuyerName.heading"),
