...
class PartnershipBuyerUtrController @Inject()(
  ...
  def onPageLoad(srn: Srn, landOrPropertyIndex: Int, disposalIndex: Int, mode: Mode): Action[AnyContent] =
    identifyAndRequireData(srn) { implicit request =>
      request.usingAnswer(PartnershipBuyerNamePage(srn, landOrPropertyIndex.refined, disposalIndex.refined)).sync {
        partnershipName =>
          val preparedForm =
            request.userAnswers
              .fillForm(PartnershipBuyerUtrPage(srn, landOrPropertyIndex.refined, disposalIndex.refined), form)
          Ok(
            view(
              preparedForm,
              PartnershipBuyerUtrController
                .viewModel(srn, landOrPropertyIndex.refined, disposalIndex.refined, mode, partnershipName)
            )
          )
      }
    }
    ...
    identifyAndRequireData(srn).async { implicit request =>
      form
        ...
        .fold(
          formWithErrors =>
            request
              .usingAnswer(PartnershipBuyerNamePage(srn, landOrPropertyIndex.refined, disposalIndex.refined))
              .async { partnershipName =>
                Future
                  .successful(
                    BadRequest(
                      view(
            ...
            for {
              updatedAnswers <- Future
                .fromTry(
                  request.userAnswers
                    .set(
                      PartnershipBuyerUtrPage(srn, landOrPropertyIndex.refined, disposalIndex.refined),
