    ...
    identifyAndRequireData(srn) { implicit request =>
      request.userAnswers
        ...
        .getOrRecoverJourney { address =>
          val preparedForm =
            request.userAnswers
              .fillForm(LandOrPropertyStillHeldPage(srn, landOrPropertyIndex.refined, disposalIndex.refined), form)
          Ok(
            view(
              preparedForm,
              viewModel(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(
                request.userAnswers
                  .set(LandOrPropertyStillHeldPage(srn, landOrPropertyIndex.refined, disposalIndex.refined), value)
              )
              nextPage = navigator
                .nextPage(
                  LandOrPropertyStillHeldPage(srn, landOrPropertyIndex.refined, disposalIndex.refined),
                  mode,
                  updatedAnswers
                )
