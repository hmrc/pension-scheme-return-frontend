...
import pages.{MemberDetails, MemberDetailsPage, RemoveMemberDetailsPage}
        ...
        .fold(
          ...
          removeMemberDetails => {
            if (removeMemberDetails) {
              for {
                updatedAnswers <- Future.fromTry(request.userAnswers.remove(RemoveMemberDetailsPage(srn)))
                _ <- saveService.save(updatedAnswers)
              } yield Redirect(navigator.nextPage(RemoveMemberDetailsPage(srn), mode, updatedAnswers))
            } else {
              Future
...
object RemoveMemberDetailsController {
  ...
  def viewModel(srn: Srn, index: Max99, nameDOB: NameDOB, mode: Mode): YesNoPageViewModel = YesNoPageViewModel(
    ...
    Message("removeMemberDetails.heading", nameDOB.lastName),
