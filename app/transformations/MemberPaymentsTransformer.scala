...
import pages.nonsipp.memberdetails.{MemberDetailsNinoPage, MemberDetailsPage, MembersDetailsMapPages, NoNINOPage}
...
class MemberPaymentsTransformer @Inject()() extends Transformer {
  ...
  def transformToEtmp(srn: Srn, userAnswers: UserAnswers): Option[MemberPayments] = {
    ...
    println(s"============== from ua ${refinedMemberDetails}")
    ...
    val memberDetails = refinedMemberDetails
      .map {
        case (index, memberDetails) =>
          ...
          println(s"============== sec map ${userAnswers.map(EmployerContributionsCompletedForMember(srn, index))}")
          println(s"============== sec ind ${userAnswers.get(EmployerContributionsCompleted(srn, index, refineMV(1)))}")
  ...
  def transformFromEtmp(userAnswers: UserAnswers, srn: Srn, memberPayments: MemberPayments): Option[Try[UserAnswers]] =
    memberPayments.memberDetails.zipWithIndex
      ...
      .map(_.foldLeft(Try(userAnswers)) {
  ...
  private def buildMemberPersonalDetails(
    ...
    MemberPersonalDetails(
      ...
      nino = maybeNino,
      noNinoReason = maybeNoNinoReason,
